 ...
 */
@Component
public class AsyncLogRateLimitPool {

    private Logger logger = LogManager.getLogger(this.getClass());

    private final ThreadPoolBulkhead AsyncLogThreadPool;
    private final RateLimiter AsyncLogRateLimiter;
    ...
    public AsyncLogRateLimitPool() {
        ...
        RateLimiterConfig limiterConfig = RateLimiterConfig.custom()
                ...
                .build();
        AsyncLogRateLimiter = RateLimiter.of("httpRequestLimiter", limiterConfig);

        // 初始化线程池
        ThreadPoolBulkheadConfig httpRequestThreadPoolConfig = ThreadPoolBulkheadConfig.custom()
                .maxThreadPoolSize(maxThread)
                .coreThreadPoolSize(coreThread)
                .queueCapacity(queueCapacity).build();
        AsyncLogThreadPool = ThreadPoolBulkhead
                .of("httpRequestThreadPool", httpRequestThreadPoolConfig);
    }
    ...
    public void dealSupplier(Supplier<Integer> supplier) {
        ...
        Decorators.ofSupplier(supplier)
            .withRateLimiter(AsyncLogRateLimiter)
            .withThreadPoolBulkhead(AsyncLogThreadPool)
